...
namespace geekeyelab {
           ...
           const int          device_id)
  {
    ...
    // data copy
    //std::cout<<"reset."<<std::endl;
    _net_dl.reset(new caffe::Net<float>(model_file));
  ...
  int GeekeyeDLL::predict(const std::vector < IplImage* >& img_dl, std::vector< std::pair<int, float> >& results)
  {
    ...
    rt.end();
    std::cout<<"forward time:"<<rt.time()<<" "<<results[0].first<<" - "<<results[0].second<<std::endl;
  ...
  int GeekeyeDLL::predict(const unsigned char* bgr, int width, int height, std::vector< std::pair<int, float> >& results)
  {
      ...
      
      {
          for (int h = 0; h < _input_blob_height; ++h) {
              for (int w = 0; w < _input_blob_width; ++w) {
                  for (int c = 0; c < _input_blob_channel; ++c) {
                      pixel = (float)bgr[h*_input_blob_width*_input_blob_channel + w*_input_blob_channel + c];
  ...
  //
  int GeekeyeDLL::get_layer_features(const std::string& imf, const std::string& layer_name, std::vector< std::vector<float> >& feat_all)
  {
    int ret = -1;
    /*
    IplImage *img = cvLoadImage(imf.c_str(), 1);
    ...
    //
    ret = get_layer_features(img, layer_name, feat_all);
    cvReleaseImage(&img);
    */
    return ret;
  }
  ...
  //
  int GeekeyeDLL::get_layer_features(const IplImage* image, const std::string& layer_name, std::vector< std::vector<float> >& feat_all)
  {
    int ret = 0;
    std::vector < IplImage* > img_dl;
    gen_one_blob(image, img_dl);
    ...
    //
    ret = get_layer_features(img_dl, layer_name, feat_all);
  ...
  int GeekeyeDLL::image_to_blob_mean_value( const std::vector<IplImage* > &img_dl, caffe::Blob<float>& image_blob)
  {
	...
	for ( h = 0; h < _input_blob_height; ++h) {
	  for ( w = 0; w < _input_blob_width; ++w) {
	    pixel = (float)((uchar*)(img_dl[d]->imageData + img_dl[d]->widthStep*h))[w*img_dl[d]->nChannels+c];
...
///
  ...
  int GeekeyeDLL::get_layer_params(const std::string& layer_name, std::vector< std::vector<float> >& params_all)
  {
   ...
   //============================================================
  //
  int GeekeyeDLL::predict(const std::string& imf, std::vector< std::pair<int, float> >& results, std::vector<cv::Rect>& rects)
  {
    int ret = -1;
    /*
    IplImage *img = cvLoadImage(imf.c_str(), 1);
  ...
  //
  int GeekeyeDLL::predict(const IplImage* image, std::vector< std::pair<int, float> >& results, std::vector<cv::Rect>& rects)
  {
  ...
  //
  int GeekeyeDLL::predict(const std::vector < IplImage* >& img_dl, std::vector< std::pair<int, float> >& results, std::vector<cv::Rect>& rects)
  {
  ...
  for (int k = 0; k < _fc_output_num; ++k ) 
    ...
    // weights LR
    const std::vector<caffe::shared_ptr<caffe::Blob<float> > >& param_blobs = _net_dl->layer_by_name("conv10")->blobs();
    ...
    caffe::shared_ptr<caffe::Blob<float> > feature_blob;
    feature_blob = _net_dl->blob_by_name("fire9/concat");
    ...
    cv::Mat map_b;
    cv::resize(map_a, map_b, cv::Size(256, 256), 0, 0, CV_INTER_LINEAR);
    //cv::imwrite("heatmap.jpg", map_b);

    //
    cv::Mat map_c;
    double minVal, maxVal;
    cv::minMaxLoc(map_b, &minVal, &maxVal);
    std::cout<<"["<<minVal<<","<<maxVal<<"]"<<std::endl;
#ifdef ANDROID_LOG
    __android_log_print(ANDROID_LOG_INFO, "det", "idx=%d score=%f min=%f max=%f",  results[0].first,  results[0].second, minVal, maxVal);
#endif
    if(maxVal > 0.0f )//&& results[0].first < 5 && results[0].second > 0.3)
    {
    cv::threshold(map_b, map_c, maxVal*.5, maxVal, CV_THRESH_BINARY);
