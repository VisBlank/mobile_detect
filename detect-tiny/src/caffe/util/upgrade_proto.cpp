...
void ReadNetParamsFromTextStringOrDie(const string& param_string,
                                    NetParameter* param) {
  ReadProtoFromTextString(param_string, param);
  UpgradeNetAsNeeded("<from param string>", param);
}
...
void ReadNetParamsFromBinaryFileOrDie(const string& param_file,
                                      NetParameter* param) {
  bool flag = ReadProtoFromBinaryFile(param_file, param);
  if(!flag)   std::cout << "Failed to parse NetParameter file: " << param_file;
#ifdef ANDROID_LOG
  if(!flag) 
    __android_log_print(ANDROID_LOG_ERROR, "init",  "Failed to parse NetParameter file: %s", param_file.c_str());
#endif
  
  flag = UpgradeNetAsNeeded(param_file, param);
#ifdef ANDROID_LOG
  if(flag) 
    __android_log_print(ANDROID_LOG_ERROR, "init",  "UpgradeNetAsNeeded:%d", (int)flag);
#endif
