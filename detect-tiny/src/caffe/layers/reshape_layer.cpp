...
/*  
  ...
  const BlobShape& top_blob_shape = t_reshape_param.shape();
  ...
  //add by chigo for write release model
  axis_ = t_reshape_param.axis();
  num_axes_ = t_reshape_param.num_axes();
  shape_.clear();
  for (i = 0; i < top_num_axes; ++i) {
  	shape_.push_back(top_blob_shape.dim(i));
  }
  */
	...
	//printf("ReshapeLayer SetUp start...\n");

	const int top_num_axes = shape_.size();
	constant_count_ = 1;
	for (i = 0; i < top_num_axes; ++i) {
		const int top_dim = shape_[i];
		if (top_dim == 0) {
			copy_axes_.push_back(i);
		} else if (top_dim == -1) {
			inferred_axis_ = i;
...
void ReshapeLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  //printf("ReshapeLayer Reshape start...\n");
  
  //const int input_start_axis = this->layer_param_.reshape_param().axis();
  //const int input_start_axis = t_reshape_param.axis();
  ...
  //const int num_axes = t_reshape_param.num_axes();
  ...
  const int num_new_axes = shape_.size();
  ...
  for (int i = 0; i < num_new_axes; ++i) {
    ...
    top_shape[top_shape_index++] = shape_[i];
  }
  ...
  //printf("ReshapeLayer Reshape end!!\n");
}
