...
/*
  ...
  ProposalParameter param = proposal_param;
  ...
  ratios_.clear();
  for (int i = 0; i < param.ratio_size(); ++i) {
  	ratios_.push_back(param.ratio(i));
  }

  scales_.clear();
  for (int i = 0; i < param.scale_size(); ++i) {
  	scales_.push_back(param.scale(i));
  }
  */

	//printf("ProposalLayer SetUp start...\n");

	vector<int> anchors_shape(2);
	anchors_shape[0] = ratios_.size() * scales_.size();
	anchors_shape[1] = 4;
	anchors_.Reshape(anchors_shape);
	generate_anchors(base_size_, &ratios_[0], &scales_[0],
					 ratios_.size(), scales_.size(),
					 anchors_.mutable_cpu_data());
	...
	//printf("ProposalLayer SetUp end!!\n");
}
...
void ProposalLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
                                       const vector<Blob<Dtype>*>& top)
{
  //printf("ProposalLayer Forward_cpu start...\n");
  ...
  //printf("ProposalLayer Forward_cpu end!!\n");
}
