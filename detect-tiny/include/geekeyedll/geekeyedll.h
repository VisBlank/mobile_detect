...
namespace geekeyelab{
  ...
  GeekeyeDLL():_use_gpu(1),_device_id(0),_layer_name("fc7"),_init_flag(false),
    ...
    /// @brief
    int predict(const std::string& imf, std::vector< std::pair<int, float> >& results, std::vector<cv::Rect>& rects);
    int predict(const IplImage* image, std::vector< std::pair<int, float> >& results, std::vector<cv::Rect>& rects);
    int predict(const std::vector < IplImage* >& img_dl, std::vector< std::pair<int, float> >& results, std::vector<cv::Rect>& rects);
    /// @brief
    ...
    /// @brief
    int get_layer_features(const std::string& imf, const std::string& layer_name, std::vector< std::vector<float> >& feat_all);
    int get_layer_features(const IplImage* image, const std::string& layer_name, std::vector< std::vector<float> >& feat_all);
