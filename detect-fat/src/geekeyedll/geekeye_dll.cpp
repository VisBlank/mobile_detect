...
namespace geekeyelab {
  ...
  int GeekeyeDLL::init(const std::string& deploy_prototxt, const std::string& caffe_model)
  {
      // set gpu device
      _use_gpu = 0;
      _device_id = 0;
...
int GeekeyeDLL::init_release(const std::string& deploy_prototxt, const std::string& caffe_model)
{
    ...
    net_dl_2.reset(new caffe::Net<float>(deploy_prototxt, caffe::TEST));
    std::cout<<"copy."<<std::endl;
    net_dl_2->CopyTrainedLayersFrom( caffe_model );
    std::cout<<deploy_prototxt<<" "<<caffe_model<<std::endl;
  ...
  int GeekeyeDLL::save_net(const std::string& model_file)
  {
     //_net_dl->SaveNet(model_file);
     //_net_dl->SaveNet_int8(model_file);

	 //_net_dl->SaveNet_detect(model_file);
	 _net_dl->SaveNet_detect_int8(model_file);
     return 0;
  }
