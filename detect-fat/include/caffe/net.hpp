...
class Net {
 ...
 public:
  ...
  //model_type:1-Uint8,0-Dtype;
  explicit Net(const string& model_file, const int model_type = 0,
  	const int level = 0, const vector<string>* stages = NULL, 
  	const Net* root_net = NULL);
...
*/
  ...
  int LoadTrainedLayers(const std::string& compress_file, Phase phase, 
  		const int level = 0, const vector<string>* stages = NULL);
  int SaveTrainedLayers(const std::string& deploy_file, const std::string& compress_file);

  // ------ add by chigo, 2016-12-26 ------
  int LoadTrainedLayers_UINT8(const std::string& compress_file, Phase phase, 
		const int level = 0, const vector<string>* stages = NULL);
  int SaveTrainedLayers_UINT8(const std::string& deploy_file, const std::string& compress_file);
  
  //save net for release 
  void SaveNet(const std::string& m_file);
  void SaveNet_int8(const std::string& m_file);
  void SaveNet_detect(const std::string& m_file);
  void SaveNet_detect_int8(const std::string& m_file);
  //load net
  void CopyFrom(const shared_ptr<Net<Dtype> >& other);
  void CopyFrom_detect(const shared_ptr<Net<Dtype> >& other);
  ...
  template <typename T>
  static inline void write_real(std::ostream &os, const T var)
  {
  ...
  template <typename T>
  static inline void write_real(std::ostream &os, const T *var, uint64_t len)
  {
    uint64_t sz = sizeof(T) * len;
    os.write( reinterpret_cast<const char *>(var), sz);
  }
