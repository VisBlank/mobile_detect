		...
		const int 		deviceID )							//[In]:GPU ID
{
	...
	if ( (!tPath1) || (!tPath2) || (!tPath3) || (!tPath4) )
	{
		...
		cout<<"[API_FACE_ANNOATION::Init:Path Err]"<<endl;
		return -1;
	}
	...
	if ( !net_dl->has_blob( strLayerName ) ) {
		...
		cout<<"[API_FACE_ANNOATION::Init:Unknown label name in the network]"<<endl;
		return -1;
	}
	...
	{	
		...
		cout<<"[API_FACE_ANNOATION::Init:imgMean Err]"<<endl;
	   	return -1;
	}	
	...
	{	
		...
		cout<<"[API_FACE_ANNOATION::Init:imgSTD Err]"<<endl;
	   	return -1;
	}	
...
int API_MUTI_LABEL::Face_ReadImageToBlob( IplImage *image, Blob<float>& image_blob)
{
	if( !image )
	{
		...
		cout<<"[API_FACE_ANNOATION::Face_ReadImageToBlob:image Err]"<<endl;
		return -1;
	}
    ...
    for ( c = 0; c < CHANNEL; ++c) {
        for ( h = 0; h < FACE_BLOB_HEIGHT; ++h) {
            for ( w = 0; w < FACE_BLOB_WIDTH; ++w) {
				tmp = (float)((uchar*)(image->imageData + image->widthStep*h))[w*channel+c];
				mean = (float)((uchar*)(imgMean->imageData + imgMean->widthStep*h))[w*channel+c];
				std = (float)((uchar*)(imgSTD->imageData + imgSTD->widthStep*h))[w*channel+c];
...
{	
	...
	float w_ratio = 1.0;
	float h_ratio = 1.0;
	IplImage* imgResize = ResizeImg( image, w_ratio, h_ratio, IMAGE_SIZE );	//320:30ms,512:50ms,720:80ms
...
IplImage* API_MUTI_LABEL::ResizeImg( IplImage *img, float &w_ratio, float &h_ratio, int MaxLen )
{
	...
	IplImage *imgResize = cvCreateImage(cvSize(rWidth, rHeight), depth, channel);
...
{	
	...
	if (Res_FaceDetect.size()<1)
	{
		...
		cout<<"[Predict: no face detect!!]"<<endl;
		return 0;
	}
...
/*	vector< FaceDetectInfo > AddFaceInfo;
	...
	for (i=0;i<Res_FaceDetect.size();i++)
	{
		...
		if ( Face_ReadImageToBlob( MutiROIResize, image_blob  ) )
		{
			...
			cout<<"[API_FACE_ANNOATION::Predict:Face_ReadImageToBlob Err!!]"<<endl;
		...
		if( batch_size < 1 )
		{
			...
			cout<<"[API_FACE_ANNOATION::Predict:batch_size err!!]"<<endl;
			if (MutiROI) {cvReleaseImage(&MutiROI);MutiROI = 0;}
			if (MutiROIResize) {cvReleaseImage(&MutiROIResize);MutiROIResize = 0;}
			return -1;
		}
