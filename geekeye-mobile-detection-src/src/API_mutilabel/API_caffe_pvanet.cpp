	...
	vector<FasterRCNNInfo_MULTILABEL> 		&vecResInfo)		//[Out]:Res
{
    ...
    for ( h = 0; h < height; ++h) {
        for ( w = 0; w < width; ++w) {
			data_buf[(         h)*width+w] = (float)((uchar*)(image->imageData + image->widthStep*h))[w*channel  ]-float(102.9801);
            data_buf[(1*height+h)*width+w] = (float)((uchar*)(image->imageData + image->widthStep*h))[w*channel+1]-float(115.9465);
            data_buf[(2*height+h)*width+w] = (float)((uchar*)(image->imageData + image->widthStep*h))[w*channel+2]-float(122.7717);
        }
    }
    im_info[0] = height;
    im_info[1] = width;
