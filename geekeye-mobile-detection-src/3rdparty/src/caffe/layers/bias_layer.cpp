...
void BiasLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if (bottom.size() == 1 && this->blobs_.size() > 0) {
    ...
    std::cout                       << "or -1 to extend to the end of bottom[0]";
    if (num_axes >= 0) {
      //CHECK_GE(bottom[0]->num_axes(), axis + num_axes)
      //std::cout    << "bias blob's shape extends past bottom[0]'s shape when applied "
...
void BiasLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  //std::cout      << "dimension mismatch between bottom[0]->shape(" << axis + i
  //      << ") and bias->shape(" << i << ")";
  }
