...
Dtype Net<Dtype>::ForwardFromTo(int start, int end) {
  ...
  for (int i = start; i <= end; ++i) {
    ...
    //std::cout<<i<<"-th:"<<layer_names_[i].c_str()<<std::endl;
    Dtype layer_loss = layers_[i]->Forward(bottom_vecs_[i], top_vecs_[i]);
    loss += layer_loss;
    //if (debug_info_)  
    // ForwardDebugInfo(i); 
    //std::cout<<layer_names_[i].c_str()<<" "<<layer_loss<<std::endl;
    //#ifdef ANDROID_LOG
    //    __android_log_print(ANDROID_LOG_ERROR, "init",  "Forwarding: %s %f", layer_names_[i].c_str(), layer_loss);
    //#endif
    //for(int k = 0; k<net_output_blobs_[0]->count(); k++)
    //std::cout<<"<"<<k<<","<<net_output_blobs_[0]->cpu_data()[k]<<">";
    //std::cout<<std::endl;
    /*
    for(int k = 0; k<bottom_vecs_[i][0]->count(); k++)
      std::cout<<"<"<<k<<","<<bottom_vecs_[i][0]->cpu_data()[k]<<">";
    std::cout<<std::endl;
    for(int k = 0; k<top_vecs_[i][0]->count(); k++)
      std::cout<<"<"<<k<<","<<top_vecs_[i][0]->cpu_data()[k]<<">";
    std::cout<<std::endl;
    std::cout<<std::endl;
    */
    /*
    int k = 100;
    float data_abs_val_mean = bottom_vecs_[i][0]->asum_data() / bottom_vecs_[i][0]->count();
    std::cout<<"bottom mean:"<<data_abs_val_mean;
    std::cout<<" <"<<k<<","<<bottom_vecs_[i][0]->cpu_data()[k]<<">";
    std::cout<<" <"<<k<<","<<top_vecs_[i][0]->cpu_data()[k]<<">"<<std::endl;
    
#ifdef ANDROID_LOG
    __android_log_print(ANDROID_LOG_ERROR, "ForwardFromTo", "%d-th: %s <%d,%f,%f>",  
      i, layer_names_[i].c_str(), k, bottom_vecs_[i][0]->cpu_data()[k],top_vecs_[i][0]->cpu_data()[k] ); 
#endif
*/
...
void Net<Dtype>::InputDebugInfo(const int input_id) {
  ...
  //LOG_IF(INFO, Caffe::root_solver())
  std::cout    << "    [Forward] "
      << "Input " << blob_name << " data: " << data_abs_val_mean<<std::endl;
  #ifdef ANDROID_LOG
        __android_log_print(ANDROID_LOG_ERROR, "init",  "[Forward] input %s,  data %f", 
          blob_name.c_str(), data_abs_val_mean);
    #endif
}
...
void Net<Dtype>::ForwardDebugInfo(const int layer_id) {
  for (int top_id = 0; top_id < top_vecs_[layer_id].size(); ++top_id) {
    ...
    #ifdef ANDROID_LOG
        __android_log_print(ANDROID_LOG_ERROR, "init",  "[Forward] layer %s, top blob %s, data %f", 
          layer_names_[layer_id].c_str(), blob_name.c_str(), data_abs_val_mean);
    #endif
  }
  for (int param_id = 0; param_id < layers_[layer_id]->blobs().size();
       ++param_id) {
    ...
    std::cout    << "    [Forward] "
        ...
        << " data: " << data_abs_val_mean<<std::endl;
    #ifdef ANDROID_LOG
        __android_log_print(ANDROID_LOG_ERROR, "init",  "[Forward] layer %s, param blob %s, data %f", 
          layer_names_[layer_id].c_str(), blob_name.c_str(), data_abs_val_mean);
    #endif
  }
}
  ...
  for (int i = 0; i < num_source_layers; ++i) {
...
#ifdef ANDROID_LOG
    __android_log_print(ANDROID_LOG_ERROR, "init",  "layer id:%d, name:%s ", i, source_layer_name.c_str());
#endif
    if (target_layer_id == layer_names_.size()) {
      //LOG(INFO) << "Ignoring source layer " << source_layer_name;
      continue;
    ...
    for (int j = 0; j < target_blobs.size(); ++j) {
      if (!target_blobs[j]->ShapeEquals(source_layer.blobs(j))) {
        ...
        #ifdef ANDROID_LOG
        __android_log_print(ANDROID_LOG_ERROR, "init",  "Cannot copy param %d  weights from layer %s", j, source_layer_name.c_str());
        #endif
      }
